...
/*
Hierarchical Clusterer of trips (Origin,destination)

In the case of Sydney B-lines

Scheduling passengers trips from and to public transit.

Simplifications of the real world problem:
 	- the area of operation is partitioned into sub-areas served by one or more shuttles
	- the service of each shuttle  is limited to a sub-area
	- each sub-area served contains several public transit stations

Given on-demand passengers' trip requests, from and to public transit, the general problem consists of:
scheduling the routes of the shuttles  satisfy global objectives.

Problem specification 1)
	Passenger's trip request to public transit can be satisfied by a drop-off at different bus stations in the sub-area



A) Split the problem in 2 sub problems
	Organising travels from passengers' origin to public transit.
	Organising travels from public transit to passengers' destination.





an arc a_i models a trip request i and consists of:
	o_i an origin location
	d_i a destination location

	es_i a earlier start time
	la_i a latest arrival time


*/

/*
 Greddy algorithm for clustering:

 	sequential path pattern : (ai, aj, ak) -> (o_i,d_i,o_j)
 	timeA (a_1,a_2) = min (
 		time( (o_1,d_1) + (d_1,o_2) + (o_2,d_2) )
 		time( (o_2,d_2)+(d_2,o_1)+(o_1,d_1) )
 	)

 	origin first path (o_i,o_j,o_k,d_i,d_j,d_k)
 	timeB (a_1,a_2) = min (
 		 time( (o_1,o_2)+(o_2,d_2)+(d_2,d_1) )
 		 time( (o_1,o_2)+(o_2,d_1)+(d_1,d_2) )
 		 time( (o_2,o_1)+(o_1,d_2)+(d_2,d_1) )
 		 time( (o_2,o_1)+(o_1,d_1)+(d_1,d_2) )
 	)
...
//			System.out.println("\n\t curCluster "+curCluster);
			...
			// 	i.e., Check that nvClustroid is  within the radius limit of all nodes in curCluster
